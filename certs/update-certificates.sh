...
# Two sources might be:
#  1. `certifi` package (e.g., C:\Users\${USER_NAME}\.conda\envs\fastapi_dev39\Lib\site-packages\certifi)\cacert.pem)
#  2. corporate pip install that works with proxy and company pem files (e.g. C:\ProgramData\PIP\${COMPANY_NAME}CAchain.pem)
#  3. manual concatination of above, and other pem files into one file (e.g. cacert_companyCAchain.pem)
#
# Conda seems to default to #1 which means you will not be able to get
# https requests (i.e., you will get a )
#
